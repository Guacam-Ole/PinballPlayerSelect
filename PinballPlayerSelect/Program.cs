...
namespace PPS
{
    internal static class Program
    {
        ...
        private static void Main(string[] args)
        {
            ...
            {
                OutputHelper.ShowMessage(logger, "Running in test mode (Won't try to open pinball, just display the parameters)");
                runInTestMode = true;
                emulator = args[1];
                tableName = args[2];
            ...
            catch (Exception ex)
            {
                ...
                OutputHelper.ShowMessage(logger, "Sorry. Something went wrong. Please check the logs");
            }
        }
